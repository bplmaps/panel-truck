<template>
  <div id="app">
    <div v-if="inError" id="error">
      <font-awesome-icon icon="exclamation-triangle"></font-awesome-icon> Sorry,
      ...
      <figure>
        ...
        >
          ...
          <div id="caption-center">
            ...
            <div
              v-if="
                currentScene >= 0 && currentScene < screenplay.scenes.length
              "
...
export default {
  ...
  data: function () {
    return {
      ...
      captionShown: true,
      currentMaxExtent: [],
      fullScreenIcon: "expand-arrows-alt",
    };
  },
  computed: {
    captionCenter: function () {
      if (this.currentScene < 0) {
        ...
        if (this.screenplay.metadata.subtitle) {
          c += `<h2 class="subtitle">${this.screenplay.metadata.subtitle}</h2>`;
        }
        if (this.screenplay.metadata.author) {
          c += `<h3 class="author">by ${this.screenplay.metadata.author}</h3>`;
        }
        return c;
      } else if (this.currentScene >= this.screenplay.scenes.length) {
        let c = `<p><strong>${this.screenplay.metadata.title}</strong></p>`;
        if (this.screenplay.metadata.author) {
          c += `<p>by ${this.screenplay.metadata.author}</p>`;
        }
        if (this.screenplay.metadata.publishedDate) {
          c += `<p>published on ${this.screenplay.metadata.publishedDate}</p>`;
        }
        ...
        if (thisScene.caption.title && thisScene.caption.title.length > 0) {
          c += `<h4 class="caption-title">${md.render(
            thisScene.caption.title
          )}</h4>`;
        }
    ...
    contextUrl: function () {
      if (
        !this.validated ||
        this.currentScene < 0 ||
        this.curentScene >= this.screenplay.scenes.length
      ) {
        return "";
      } else {
        return this.screenplay.scenes[this.currentScene].moreInfo
          ? this.screenplay.scenes[this.currentScene].moreInfo
          : this.screenplay.sources.filter((d) => {
              return d.id === this.screenplay.scenes[this.currentScene].source;
            })[0].moreInfo
          ? this.screenplay.sources.filter((d) => {
              return d.id === this.screenplay.scenes[this.currentScene].source;
            })[0].moreInfo
  ...
  methods: {
    toggleFullscreen: function () {
      ...
      if (!isInFullScreen) {
        ...
        this.fullScreenIcon = "compress-arrows-alt";
      } else {
    ...
    changeScene: function (to, initial) {
      ...
      if (this.currentScene === -1) {
        ...
        currentSceneSource =
          this.screenplay.scenes[this.currentScene - 1].source;
      } else {
      ...
      if (
        initial ||
        currentSceneSource != this.screenplay.scenes[nextFunctionalScene].source
      ) {
        this.changeSource(nextFunctionalScene).then(() => {
          this.changeExtent(nextFunctionalScene);
        });
      } else {
    ...
    changeSource: function (sceneNumber) {
      return new Promise((resolve, reject) => {
        let newSource = this.screenplay.sources.filter((d) => {
          return d.id === this.screenplay.scenes[sceneNumber].source;
        })[0];
        if (newSource.type.toLowerCase() === "iiifmanifest") {
          ...
          fetch(newSource.manifest)
            ...
            .then((m) => {
              let imageEndpoint =
                m.sequences[sequence].canvases[canvas].images[image].resource
                  .service["@id"];
              this.loadNewIIIF(imageEndpoint).then(resolve).catch(reject);
            })
            .catch(reject);
        } else if (newSource.type.toLowerCase() === "iiifimage") {
          this.loadNewIIIF(newSource.imageEndpoint).then(resolve).catch(reject);
        } else if (newSource.type.toLowerCase() === "staticimage") {
          this.loadNewImage(newSource.imageFile).then(resolve).catch(reject);
        } else if (newSource.type.toLowerCase() === "geomap") {
          this.loadNewGeoMap(newSource).then(resolve).catch(reject);
        } else {
          reject();
        }
      });
    ...
    changeExtent: function (sceneNumber, transitionDuration) {
      let extent = this.screenplay.scenes[sceneNumber].extent
        ? this.screenplay.scenes[sceneNumber].extent
        : "fit";
      let bottomPadding =
        this.$refs.captionWrapper.clientHeight > 50
          ? this.$refs.captionWrapper.clientHeight + 20
          : 70;
      let fitOptions = { padding: [10, 10, bottomPadding, 10] };
      if (typeof transitionDuration === "number") {
        fitOptions.duration = transitionDuration;
      }
